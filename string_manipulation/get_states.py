...
def get_every_nth_state(states=states, n=10):
    ...
    return [state for d, state in enumerate(states, 1) if d%(n)==0]
...
def get_state_abbrev(state_name, us_state_abbrev=us_state_abbrev):
    ...
    if state_name not in us_state_abbrev.keys():
        ...
        return [v for k , v in us_state_abbrev.items() if state_name == k][0]
...
def get_longest_state(data):
    ...
    if data == us_state_abbrev:
        data = us_state_abbrev.keys()
    length = max([len(elem) for elem in data])
    return [state for state in data if len(state)==length][0]
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,
    ...
    abbreviations = [ab for n, ab in enumerate(sorted(us_state_abbrev.values()), 1) if n<=10]
    state = sorted(states)[-10:]
