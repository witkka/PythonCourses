...
def operas_both_at_premiere(guest, composer):
    ...
    """
    list_comp = [x for x in composers ]
    if guest not in list_comp or composer not in list_comp:
        raise ValueError
    else: 
        titles = []

        guest_dates = [(_get_date(c[1].born), _get_date(c[1].died)) for c in composers.items() if c[0] == guest]
        composer_dates = [(_get_date(c[1].born), _get_date(c[1].died)) for c in composers.items() if c[0] == composer]
        for o in operas:
            if o.author == composer:
                if _get_date(o.date) < composer_dates[0][1]:

                    if _get_date(o.date)>guest_dates[0][0] and _get_date(o.date)<guest_dates[0][1]:
                    
                        titles.append(o.play)
                else:
                    continue
    return titles